...
const GraphChat: React.FC<GraphChatProps> = ({ conversationId: _conversationId }) => {
  ...
  // Fix: Get both refetch functions
  const { data: messageEdges, isFetched: edgesAreFetched, refetch: refetchEdges } = useGetConversationEdges(
    conversationId!, 
    {
  ...
  const { data: messages, isFetched: messagesAreFetched, refetch: refetchMessages } = useGetConversationMessages(
    conversationId!, 
    {
      enabled: !!conversationId && conversationId > 0,
    }
  ...
  // Add this useEffect to manually trigger refetch when conversation ID changes
  useEffect(() => {
    if (conversationId && conversationId > 0) {
      console.log('Manually triggering refetch for conversation:', conversationId);
      refetchMessages(); // Now this is the correct function
      refetchEdges();
    }
  }, [conversationId, refetchMessages, refetchEdges]);
  ...
  console.log('createPrompt hook:', {
    mutate: typeof createPrompt.mutate,
  ...
  const onSendPrompt: OnSendPrompt = useCallback(
    ({ id, content, position }) => {
      console.log('ACTIVE onSendPrompt called with:', { id, content, position });
      ...
      createPrompt.mutate(
        ...
        },
        {
          onSuccess: (data) => {
            ...
            setConversationId(data.conversationId); // This line is crucial!
            // Add a small delay and then force refetch
            setTimeout(() => {
              console.log('🔄 Force refetching after conversation creation');
              refetchMessages();
              refetchEdges();
            }, 100);
          },
    ...
    [nodes, createPrompt, edges, setNodes, setEdges, conversationId, refetchMessages, refetchEdges]
  );
  ...
  //               applyEdgeChanges(
  //                 [
  //                   // Add the new edges - only if createdEdges exists
